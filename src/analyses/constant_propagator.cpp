  ...
  const namespacet &ns)
{
  ...
  else if(from->is_function_call())
  {
    ...
    if(function.id()==ID_symbol)
    {
      ...
      {
        if(
          id == CPROVER_PREFIX "set_must" || id == CPROVER_PREFIX "get_must" ||
          id == CPROVER_PREFIX "set_may" || id == CPROVER_PREFIX "get_may" ||
          id == CPROVER_PREFIX "cleanup" || id == CPROVER_PREFIX "clear_may" ||
...
bool constant_propagator_domaint::valuest::meet(const valuest &src)
{
  ...
  for(const auto &m : src.replace_const.expr_map)
  {
    replace_symbolt::expr_mapt::iterator c_it =
      replace_const.expr_map.find(m.first);
